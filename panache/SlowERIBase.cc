...
void SlowERIBase::free_array(double* array)
{
    delete[] array;
}


SlowERIBase::SlowERIBase()
{
    int i = 0;
    int j = 0;

    fac = new double[MAXFAC];
    fac[0] = 1.0;
    for (i = 1; i < MAXFAC; i++)
        fac[i] = fac[i - 1] * i;

    bc = new double*[MAXFAC];
    for (i = 0; i < MAXFAC; i++)
    {
        bc[i] = new double[MAXFAC];

        for (j = 0; j <= i; j++)
            bc[i][j] = fac[i] / (fac[i - j] * fac[j]);
    }

    df = new double[2*MAXFAC];
    df[0] = 1.0;
    df[1] = 1.0;
    df[2] = 1.0;
 ...
 */
void SlowERIBase::calc_f(double *F, int n, double t)
{
    int i, m, k;
    int m2;
    double t2;
