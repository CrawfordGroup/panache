...
static void transform1e_1(int am, const SphericalTransform& sti, double *s, double *t, int nl)
{
...
static void transform1e_2(int am, const SphericalTransform& sti, double *s, double *t, int nk, int nl)
{
    const int sl = nl;
...
{
    throw RuntimeError("libmints: OneBodyInt::clone() Not implemented");
}
                                  ...
                                  const GaussianShell& s2, int chunks)
{
    for (int chunk=0; chunk<chunks; ++chunk) {
        ...
        if (is_pure2) {
            transform1e_2(am2, SphericalTransform::Generate(am2), source2, target2, ncart1, ncart2);
        }
        if (is_pure1) {
            transform1e_1(am1, SphericalTransform::Generate(am1), source1, target1, nbf2);
        }
...
{
    throw RuntimeError("OneBodyAOInt::compute_pair_deriv1: Not implemented.");
}
...
void OneBodyAOInt::compute(std::vector<SharedMatrix > &result)
{
    ...
    if (result.size() != nchunk_) {
        ...
        throw RuntimeError("OneBodyInt::compute(result): result incorrect length.");
    }

    // Check the individual matrices, we can only handle nirrep() == 1
    for(auto & it : result) {
        if (it->nirrep() != 1) {
            throw RuntimeError("OneBodyInt::compute(result): one or more of the matrices given has symmetry.");
        }
    }
...
{
    if (deriv_ < 1)
        throw RuntimeError("OneBodyInt::compute_deriv1(result): integral object not created to handle derivatives.");
    ...
    if (result.size() != 3*natom_)
        throw RuntimeError("OneBodyInt::compute_deriv1(result): result must be 3 * natom in length.");

    if (result[0]->nirrep() != 1)
        throw RuntimeError("OneBodyInt::compute_deriv1(result): results must be C1 symmetry.");
...
{
    if (deriv_ < 2)
        throw RuntimeError("OneBodyInt::compute_deriv2(result): integral object not created to handle derivatives.");
    ...
    if (result.size() != 9*natom_*natom_)
        throw RuntimeError("OneBodyInt::compute_deriv2(result): result must be 9 * natom^2 in length.");

    if (result[0]->nirrep() != 1)
        throw RuntimeError("OneBodyInt::compute_deriv2(result): results must be C1 symmetry.");
