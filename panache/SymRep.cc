...
SymRep::operator SymmetryOperation() const
{
  if (n != 3) {
    ...
    // abort();
      throw RuntimeError("SymRep::operator SymmetryOperation(): trying to cast to symop when n != 3");
  }
...
SymRep::operate(const SymRep& r) const
{
  if (r.n != n) {
    ...
    //      << r.n << " != " << n << endl;
    // abort();
      throw RuntimeError("SymRep::operate(): dimensions don't match");
  }
...
SymRep::rotation(double theta)
{
  ...
  default:
    ...
    // abort();
    throw RuntimeError("SymRep::rotation(): n > 5");
  }
